---
date: 2026-09-23T19:06:33-0300
topic: diagramas-cifra-f2-transpose-reading-codex
artifact: 4c1a21d..fb9e228
skill: review-code
...
# Cross-Model Review — diagramas-cifra-f2-transpose-reading-codex
...
The new absolute-key output remains subject to relative-versus-absolute inference when resolved or transposed again. This introduces a verified regression that changes displayed notes, breaks reverse transposition, and persists through CHO export and reimport.

## Findings

### F-001 [major] Correctness — src/core/define.ts:200-201
...
**Claim:** Transposing `{define: Dsus2 keys 0 7}` by `+5` produces `Gsus2 keys 7 2`, which resolves to D–A instead of the required G–D because the resolver reinterprets the emitted absolute keys as relative intervals.

**Impact:** At Gsus2, both readings score two chord-tone matches, but the relative reading wins on its characteristic second, changing `[7,2]` into `[2,9]`. Transposing back by `−5` then produces `[9,4]` (A–E), losing the original D–A voicing. `exportCho` serializes the same ambiguous representation, so reimport also displays incorrect notes. Executing both revisions confirmed that `4c1a21d` preserves the expected notes for this sequence and `fb9e228` does not.

**Recommendation:** Preserve an explicit key interpretation through transposition, resolution, and serialization so generated absolute keys cannot be guessed as relative again. Add regression coverage for the existing Dsus2 example using `+5`, reverse transposition, and export/reimport, asserting rendered pitches.
...
The constraints confirm that transposed absolute keys are interpreted again using the same heuristic. That second interpretation can change the sounding notes, breaking reverse transposition and exported overrides. The blind finding remains valid.
...
**Claim:** Transposing `{define: Dsus2 keys 0 7}` by `+5` produces `Gsus2 keys 7 2`, which resolves to D–A instead of G–D because the emitted absolute keys are reinterpreted as relative intervals.

**Impact:** Gsus2 expects pitch classes `[7,9,2]`. The absolute reading `[7,2]` and relative reading `[2,9]` each match two chord tones, but only the relative reading contains the characteristic second, so it wins. The diagram consequently displays incorrect notes. Transposing back by `−5` stores `[9,4]`, yielding A–E instead of the original D–A. Because `exportCho` calls the same rewrite, export and reimport preserve the incorrect interpretation. The added `+3` round-trip test does not exercise this failure.

**Recommendation:** Preserve an explicit key interpretation through transposition, resolution, and serialization so generated absolute keys cannot be reinterpreted as relative. Add regression coverage for this `+5` case, reverse transposition, and export/reimport, asserting rendered pitches.
...
- Vue single-file components, color, font, modal layout, and auto-scroll timing.
...
- F-001-blind → F-001-final [major] — same; the shared decision rule and sus2 characteristic interval confirm that the emitted `[7,2]` is reinterpreted as `[2,9]`.
...
index 2acfbf1..ef303b6 100644
--- a/src/core/chord-dict.ts
+++ b/src/core/chord-dict.ts
@@ -142,38 +142,42 @@ function scorePianoReadings(keys: readonly number[], rootPc: number, quality: st
...
@@ -331,6 +331,61 @@ describe('resolveDiagram', () => {
...
- pianoSoundingPitchClasses and pianoKeysToRelative share one decision: higher chord-tone score, then slash bass in exactly one reading, then characteristic-tone count, then absolute. src/core/chord-dict.ts.
- pianoKeysToRelative returns those sounding classes minus the root. drawPiano adds the root back.
- transposeDefine, for keys all inside 0–11, stores (sounding + n) % 12. MIDI keys outside 0–11 add n and do not wrap. src/core/define.ts.
- exportCho with a semitone count calls rewriteDefineLines, which calls transposeDefine.
- sus2 intervals are 0, 2, 7. The characteristic interval kept after dropping 0 and 7 is 2.
...
None during the review. The two previously reported cases were fixed before this pass. The remaining major stands.
