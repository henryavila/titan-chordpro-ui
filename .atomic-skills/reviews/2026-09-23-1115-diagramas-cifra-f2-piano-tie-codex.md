---
date: 2026-09-23T11:14:19-0300
topic: diagramas-cifra-f2-piano-tie-codex
artifact: 6812f54..d2d36c
skill: review-code
...
# Cross-Model Review — diagramas-cifra-f2-piano-tie-codex
...
The new tie-breaker can transpose an absolute piano override incorrectly, removing an explicitly named slash bass and introducing a foreign note. A targeted execution confirmed that the same input renders correctly at the base revision.

## Findings

### F-001 [major] Correctness — src/core/chord-dict.ts:138-139
...
**Claim:** Resolving `D7M(9)/B` with an absolute piano override `keys: [11, 2, 1, 4]` now renders `[1, 4, 3, 6]` instead of preserving the supplied pitches.

**Impact:** The diagram displays C#, E, D#, F# instead of B, D, C#, E, removing the specified B bass and introducing D#, which is outside Dmaj9. Both interpretations score three chord tones, but the new characteristic-tone count favors the relative interpretation, three to two. The caller supplies only root and quality, so the declared slash bass cannot disambiguate the tie. The base revision preserves the original notes.

**Recommendation:** Pass the declared bass pitch into disambiguation and prefer the interpretation that preserves it when chord-tone scores tie. Add a resolve-and-draw regression test for this absolute override alongside the new relative-key test.
...
- Vue components, visual styling, modal layout, and auto-scroll timing.
...
The external constraints confirm the blind finding: matching piano overrides reach the new tie-breaker without their declared slash bass. The tie-breaker can reinterpret absolute keys as relative intervals, removing the specified bass and introducing a foreign note. No additional findings emerge from the constraints.
...
**Claim:** Resolving `D7M(9)/B` with a matching absolute piano override `keys: [11, 2, 1, 4]` now renders `[1, 4, 3, 6]` instead of the supplied pitches.

**Impact:** The diagram displays C#, E, D#, F# instead of B, D, C#, E, removing the declared B bass and introducing D#, which is outside Dmaj9. Both interpretations score three chord tones, but the new characteristic-tone count favors relative interpretation three to two. The matching override bypasses the slash-token dictionary guard; its conversion receives no `bassPc`, and drawing does not restore the bass. The previous absolute tie fallback preserved these pitches.

**Recommendation:** Pass the declared bass pitch into disambiguation and prefer the interpretation that preserves it when chord-tone scores tie. Add a resolve-and-draw regression test for this absolute slash override.
...
- Vue components, colors, fonts, modal layout, auto-scroll timing, and unrelated files.
...
- A slash token sets bassPc from the note after /. canonicalOf is in src/core/resolve-diagram.ts.
- If bassPc is set and no override matches, resolveDiagram returns miss reason no-shape before lookupDict.
- A matching piano override with keys calls pianoKeysToRelative(keys, rootPc, quality) and does not pass bassPc.
- drawPiano lights (rootPc + stored key) % 12. It does not add the bass by itself.
...
No product edit in this review session. The major stands. The next phase was not opened.
