# Design — Som da batida no ensaio + Ensaio Batida

> **Aprovado** (usuário, 2026-09-15) · síntese pós-debate (Priya · Uma · Aria · Tariq-contrário)  
> Digest: `research-digest.md` · proposta: `proposta-ux.md`

## Decisions

1. **Fonte do ensaio** na UI: `Mudo | Click | Batida` (+ click suave opcional com Batida). Prefs SoT continuam `metSound` / `metStrumSound`.
2. **Ensaio Batida** = perfil de chrome (`rehearsalFocus: 'off' | 'batida'`), **não** `lens`.
3. **Rolar sempre silencioso** fora do Ensaio Batida (`met.start({ silent: true })`) — prefs de prática não vazam no palco.
4. **Count-in** = click/visual no primeiro compasso; **batida muda** até a cifra entrar (sem catch-up de hits). Rolar silencioso = count-in visual.
5. **Strip** abre ao entrar em Ensaio Batida; sair / trocar música reseta o modo.
